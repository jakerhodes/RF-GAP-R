...
# TODO: Optimize the function; not efficient as written
...
rf_impute <- function(x, y, type = "rfgap", n_iters = 1, x_true = NULL,
  ...
  if (exists("x_true")) {
    cat_error[1] <- 1 - sum(x_true[factor_cols] == x_med[factor_cols]) /
      prod(dim(x[factor_cols]))

    cont_error[1] <- norm(as.matrix(x_true[!factor_cols] - x_med[!factor_cols]), "F")
  }
  ...
  if (n_iters == 0) {
    ...
    for (iter in 1:n_iters) {
      ...
      prox <- get_proximities(x = x_imp, rf = rf, y = y, type = type)


      # Running Imputation
      miss_vars <- unique(miss_idx[, 2])
      for (j in miss_vars) {
        if (factor_cols[colnames(x)[j]]) {
      ...
      if (!is.null(x_true)) {
        cat_error[iter + 1] <- 1 - sum(x_true[factor_cols] == x_imp[factor_cols]) /
          prod(dim(x[factor_cols]))

        cont_error[iter + 1] <- norm(as.matrix(x_true[!factor_cols] - x_imp[!factor_cols]), "F")
      }
    } # End for iter
  } # End else
