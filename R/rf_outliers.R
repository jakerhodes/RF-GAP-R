...
#' detect_outliers produces a dataset's outlier measure score based on
...
rf_outliers <- function(x, y, type = "rfgap", rf = NULL, ...) {
  ...
  if (!is.factor(y)) {
    stop('rf_outliers is only available for classification problems.
         Be sure "y" is a factor type.')
  }
  ...
  if ('rf_proximities' %in% class(x)) {
    ...
    if (!is.null(rf)) {
      ...
      prox <- get_proximities(x, y, type = type, ...)
    }
  }
...
#' @param min_point_size starting point size
#' @param scale_factor how much is the outlier score affecting point size
#' @param ... Additional arguments for mds and generic plotting
#' @import ggplot2
#' @export
#'

plot.rf_outlier <- function(x, data, y, min_point_size = 2,
                            scale_factor = 3, ...) {
